...
def extratodetalhado(request):
    ...
    cmd_jefferson = servico1.objects.all().filter(funcionario2__nome=jeff, data__icontains=hoje)
    cmd_rubens = servico1.objects.all().filter(funcionario2__nome=rubens, data__icontains=hoje)
